...
class AccountMove(models.Model):
    ...
    def _default_journal_document_class_id(self):
        if (
            not self.env["ir.model"].search([("model", "=", "sii.document_class")])
            or self.document_class_id
        ):
            return False
        journal = self.env["account.move"].default_get(["journal_id"])["journal_id"]
        default_type = self._context.get("default_move_type", "")
        if not self.check_invoice_type(default_type) or default_type in [
            "in_invoice",
            "in_refund",
        ]:
        ...
        dc_type = (
            ["invoice"]
            if default_type in ["in_invoice", "out_invoice"]
            else ["credit_note", "debit_note"]
        )
    ...
    def get_dc_ids(self):
        for r in self:
            if not r.check_invoice_type(r.move_type):
                r.document_class_ids = False
                continue
            r.document_class_ids = []
            dc_type = (
                ["invoice"]
                if r.move_type in ["in_invoice", "out_invoice"]
                else ["credit_note", "debit_note"]
            )
    ...
    claim = fields.Selection(
        [
            ...
            (
                "ENC",
                "Recepción de NC, distinta de anulación, que referencia al documento.",
            ),
    ...
    claim_ids = fields.One2many(
        "sii.dte.claim", "move_id", strign="Historial de Reclamos"
    )
    ...
    sequence_number_next = fields.Integer(compute="_get_sequence_number_next")
    sequence_number_next_prefix = fields.Integer(compute="_get_sequence_prefix")
    ...
    def _compute_amount(self):
        for move in self:
            ...
            move.amount_retencion = sign * (
                total_retencion_currency if len(currencies) == 1 else total_retencion
            )
    ...
    def _tax_tags_need_inversion(self, move, is_refund, tax_type):
        ...
        if move.move_type == "entry":
            return (tax_type == "sale" and not is_refund) or (
    ...
    def _compute_name(self):
        ...
        for r in self:
            if not r.document_class_id or r.state == "draft":
                ...
                continue
            r.name = "%s%s" % (
                r.document_class_id.doc_code_prefix,
    ...
    def _post(self, soft=True):
        ...
        for inv in to_post:
            if (
                not inv.is_invoice()
                or not inv.journal_document_class_id
                or not inv.use_documents
            ):
                continue
            ...
            if inv.journal_id.restore_mode:
                ...
                tiempo_pasivo = datetime.now() + timedelta(
                    hours=int(
                        ...
                        .sudo()
                        .get_param("account.auto_send_dte", default=1)
                    )
                )
    ...
    def totales_por_movimiento(self):
        ...
        imps["neto"] = (
            self.amount_total - imps["otros_imps"] - imps["exento"] - imps["iva"]
        )
        return imps
    ...
    def _get_sequence_prefix(self):
        for invoice in self:
            ...
            if invoice.journal_document_class_id:
                invoice.sequence_number_next_prefix = (
                    invoice.document_class_id.doc_code_prefix or ""
                )
        ...
        for invoice in self:
            ...
            if invoice.journal_document_class_id:
                invoice.sequence_number_next = (
                    invoice.journal_document_class_id.sequence_id.number_next_actual
                )
    ...
    def _prepare_refund(
        ...
        values = super(AccountMove, self)._prepare_refund(
            invoice, invoice_date, description, journal_id
        )
                ...
                "referencias": [
                    [
                        ...
                        0,
                        {
                            ...
                            "motivo": description,
                            "fecha_documento": invoice.invoice_date.strftime(
                                "%Y-%m-%d"
                            ),
                        },
                    ]
    ...
    def action_invoice_cancel(self):
        for r in self:
            if r.sii_xml_request and r.sii_result not in [
                False,
                "draft",
                "NoEnviado",
                "Anulado",
    ...
    def unlink(self):
        ...
        for r in self:
            if r.sii_xml_request and r.sii_result in [
                "Aceptado",
                "Reparo",
                "Rechazado",
            ]:
    ...
    def _check_reference_in_invoice(self):
        if self.move_type in ["in_invoice", "in_refund"] and self.sii_document_number:
            ...
            if move_ids:
                raise UserError(
                    "El numero de factura debe ser unico por Proveedor.\n"
    ...
    def _check_duplicate_supplier_reference(self):
        for invoice in self:
            if (
                ...
                and invoice.sii_document_number
            ):
                if self.search(
                    [
                    ...
                    raise UserError(
                        ...
                        % (
                            invoice.document_class_id.name,
                            invoice.sii_document_number,
    ...
    def _validaciones_uso_dte(self):
        ...
        if self.document_class_id.sii_code in ncs + nds and not self.referencias:
            raise UserError(
                "Las Notas deben llevar por obligación una referencia al documento que están afectando"
            )
        ...
        if (
            ...
            and self.document_class_id.sii_code not in ncs
        ):
            raise UserError(
                _(
                    "El tipo de documento %s, no es de tipo Rectificativo"
                    % self.document_class_id.name
                )
            )
        if (
            self.move_type in ["out_invoice", "in_invoice"]
            and self.document_class_id.sii_code in ncs
        ):
            raise UserError(
                _(
                    "El tipo de documento %s, no es de tipo Documento"
                    % self.document_class_id.name
                )
            )
        for gd in self.global_descuentos_recargos:
            if gd.valor <= 0:
                raise UserError(
                    _(
                        "No puede ir una línea igual o menor que 0, elimine la línea o verifique el valor ingresado"
                    )
                )
    ...
    def default_journal(self):
        ...
        if self._context.get("honorarios", False):
            ...
            domain = [
                (
                    "journal_document_class_ids.sii_document_class_id.document_letter_id.name",
                    "=",
                    "M",
                ),
                (
    ...
    def _recompute_global_gdr_lines(self):
        ...
        def _apply_global_gdr(
            self, amount, amount_currency, global_gdr_line, gdr, taxes
        ):
            if self.move_type in ["in_invoice", "in_refund"]:
                amount *= -1
            if gdr.type == "D":
            ...
            if in_draft_mode:
                global_gdr_line.update(
                    global_gdr_line._get_fields_onchange_balance(force_computation=True)
                )
            if gdr.impuesto == "afectos":
        ...
        for gdr in self.global_descuentos_recargos:
            ...
            if gdr.type == "D":
                ...
                gdr_amount, gdr_amount_currency = _compute_global_gdr(
                    self, others_lines, gdr
                )
                total_gd += gdr_amount
    ...
    def _id_doc(self, taxInclude=False, MntExe=0):
        ...
        if self.es_boleta():
            IdDoc[
                "IndServicio"
            ] = 3  # @TODO agregar las otras opciones a la fichade producto servicio
        ...
        if not self.es_boleta() and self.invoice_date_due:
            IdDoc["FchVenc"] = self.invoice_date_due.strftime(
                "%Y-%m-%d"
            ) or datetime.strftime(datetime.now(), "%Y-%m-%d")
        return IdDoc
    ...
    def _emisor(self):
        ...
        if self.journal_id.sucursal_id:
            Emisor["Sucursal"] = self._acortar_str(
                self.journal_id.sucursal_id.partner_id.name, 20
            )
        ...
        Emisor["DirOrigen"] = self._acortar_str(
            dir_origen.street + " " + (dir_origen.street2 or ""), 70
        )
    ...
    def _receptor(self):
        ...
        commercial_partner_id = (
            self.commercial_partner_id or self.partner_id.commercial_partner_id
        )
        if (
            not commercial_partner_id.vat
        ...
        if (
            ...
            GiroRecep = (
                self.acteco_id.name or commercial_partner_id.activity_description.name
            )
            if not GiroRecep:
        ...
        if self.partner_id.phone or commercial_partner_id.phone:
            Receptor["Contacto"] = self._acortar_str(
                ...
                or commercial_partner_id.phone
        ...
        street2_recep = (
            self.partner_id.street2 or commercial_partner_id.street2 or False
        )
    ...
    def _totales_otra_moneda(
        self, currency_id, MntExe, MntNeto, IVA, TasaIVA, MntTotal=0, MntBase=0
    ):
        Totales = {}
        ...
        if MntNeto > 0:
            if currency_id != self.currency_id:
                MntNeto = currency_id._convert(
                    MntNeto, self.currency_id, self.company_id, self.invoice_date
                )
            Totales["MntNetoOtrMnda"] = MntNeto
        if MntExe:
            if currency_id != self.currency_id:
                MntExe = currency_id._convert(
                    MntExe, self.currency_id, self.company_id, self.invoice_date
                )
        ...
        if TasaIVA:
            if currency_id != self.currency_id:
                IVA = currency_id._convert(
                    IVA, self.currency_id, self.company_id, self.invoice_date
                )
            Totales["IVAOtrMnda"] = IVA
        if currency_id != self.currency_id:
            MntTotal = currency_id._convert(
                MntTotal, self.currency_id, self.company_id, self.invoice_date
            )
    ...
    def _es_exento(self):
        return self.document_class_id.sii_code in [32, 34, 41, 110, 111, 112] or (
            self.referencias
            and self.referencias[0].sii_referencia_TpoDocRef.sii_code in [32, 34, 41]
        )
            ...
            for t in self.line_ids:
                ...
                for tl in t.tax_ids:
                    ...
                    if tl.sii_code in [17]:
                        MntBase += (
                            t.balance
                        )  # @TODO Buscar forma de calcular la base para faenamiento
        if (
            self.amount_tax == 0
            and MntExe > 0
            and not self._es_exento()
            and self.document_class_id.sii_code not in [60, 61, 55, 56]
        ):
    ...
    def _encabezado(self, MntExe=0, no_product=False, taxInclude=False):
        ...
        MntExe, MntNeto, IVA, TasaIVA, MntTotal, MntBase, CredEC = self._totales(
            MntExe, no_product, taxInclude
        )
        Encabezado["Totales"] = self._totales_normal(
            currency_base, MntExe, MntNeto, IVA, TasaIVA, MntTotal, MntBase, CredEC
        )
    ...
    def _validaciones_caf(self, caf):
        ...
        if fecha_timbre < date(
            int(caf["FA"][:4]), int(caf["FA"][5:7]), int(caf["FA"][8:10])
        ):
            raise UserError(
                "La fecha del timbraje no puede ser menor a la fecha de emisión del CAF"
            )
        return timestamp
    ...
    def is_price_included(self):
        ...
        if tax.price_include or (
            not tax.sii_detailed and (self.es_boleta() or self.es_nc_boleta())
        ):
            return True
        return False
    ...
    def _invoice_lines(self):
        ...
        for line in self.with_context(lang="es_CL").invoice_line_ids:
            ...
            if lines["MontoItem"] < 0:
                raise UserError(
                    _("No pueden ir valores negativos en las líneas de detalle")
                )
    ...
    def _gdr(self):
        ...
        for dr in self.global_descuentos_recargos:
            ...
            if self.document_class_id.sii_code in [34] and (
                self.referencias
                and self.referencias[0].sii_referencia_TpoDocRef.sii_code == "34"
        ...
        if self.referencias:
            for ref in self.referencias:
                ...
                if not self.es_boleta():
                    ...
                    ref_line["FchRef"] = ref.fecha_documento or datetime.strftime(
                        datetime.now(), "%Y-%m-%d"
                    )
        ...
        datos["Documento"] = [
            {
                ...
                "caf_file": [
                    self.journal_document_class_id.sequence_id.get_caf_file(
                        folio, decoded=False
                    ).decode()
                ],
    ...
    def _crear_envio(self, n_atencion=None, RUTRecep="60803000-K"):
        ...
        for r in self:
            ...
            if (
                self._context.get("set_pruebas", False)
                or r.sii_result == "Rechazado"
                or not r.sii_xml_dte
            ...
            if r.sii_result in ["Rechazado"] or (
                self._context.get("set_pruebas", False)
                and r.sii_xml_request.state in ["", "draft", "NoEnviado"]
            ):
        ...
        if self._context.get("set_pruebas", False):
            api = False
        datos.update({"api": api, "RutReceptor": RUTRecep, "Documento": []})
    ...
    def ask_for_dte_status(self):
        for r in self:
            if not r.sii_xml_request and not r.sii_xml_request.sii_send_ident:
                raise UserError(
                    "No se ha enviado aún el documento, aún está en cola de envío interna en odoo"
                )
            if r.sii_xml_request.state not in ["Aceptado", "Rechazado"]:
                r.sii_xml_request.with_context(
                    set_pruebas=self._context.get("set_pruebas", False)
                ).get_send_status(r.env.user)
        try:
        ...
        for r in self:
            ...
            if mess:
                self.env["bus.bus"].sendone(
                    (self._cr.dbname, "account.move", r.user_id.partner_id.id), mess
                )
    ...
    def set_dte_claim(self, claim=False):
        ...
        try:
            ...
            key = "RUT%sT%sF%s" % (rut_emisor, tipo_dte, str(self.sii_document_number))
    ...
    def invoice_print(self):
        ...
        if self.ticket or (
            self.document_class_id and self.document_class_id.sii_code == 39
        ):
    ...
    def send_exchange(self):
        ...
        dte_email_id = (
            self.company_id.dte_email_id or self.env.user.company_id.dte_email_id
        )
    ...
    def sii_header(self):
        ...
        d.text(
            (50, 30),
            "R.U.T.: %s" % self.company_id.document_number,
            fill=(0, 0, 0),
            font=font,
        )
        d.text((50, 90), self.document_class_id.name, fill=(0, 0, 0), font=font)
        d.text(
            (220, 150), "N° %s" % self.sii_document_number, fill=(0, 0, 0), font=font
        )
        font = ImageFont.truetype(
        ...
        d.text(
            (200, 235),
            "SII %s" % self.company_id.sii_regional_office_id.name,
            fill=(0, 0, 0),
            font=font,
        )
